...
def log_execution(level=logging.DEBUG, include_args=True, include_result=True):
    """
    记录函数执行的装饰器
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if include_args:
                ...
                log_msg.append(f"{args_str!r}")
            logger.debug(f"{func_name}{log_msg!r}")
            # 记录执行时间和结果
            ...
            try:
                ...
                # 记录成功执行信息
                success_msg = f"{func_name}\t{execution_time:.2f}ms"
